// fonction qui envoie la requête à l'API et récupère la donnée du niveau d'eau :

async function fetchWaterLevel2() {
  const reponse = await fetch(
    "https://hubeau.eaufrance.fr/api/v1/hydrometrie/observations_tr?code_entite=M800001010&fields=resultat_obs&grandeur_hydro=H&size=1"
  );
...
async function fetchDate() {
  const reponse = await fetch(
    "https://hubeau.eaufrance.fr/api/v1/hydrometrie/observations_tr?code_entite=M800001010&fields=date_obs&grandeur_hydro=H&size=1"
  );
  const data = await reponse.json();
...
// fonction principale qui affiche la donnée de l'API reçue dans l'HTML sur la règle, modifie la hauteur de la vague, lance l'animation et
// l'arrête :
...
async function main() {
  ...
  document.querySelector(".seblecrab").style.top =
    (reversedLvl * 400) / 8000 + 50 + "px"; //colle le fond à la wave
  document.querySelector(".result").style.top =
    (reversedLvl * 400) / 8000 - 15 + "px"; //hauteur des résultats

  // partie transition animée :
  ...
  waveElement.addEventListener("transitionend", () => {
    document.getElementById("waterLevel").innerHTML =
      waterlVl + " mm, le " + datepropre + " à " + hourpropre;
